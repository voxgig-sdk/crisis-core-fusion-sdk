...
module CrisisCoreFusionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CrisisCoreFusionResponse.new({})
    ...
    if fetch_err
      ...
      response = CrisisCoreFusionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CrisisCoreFusionResponse.new(fetched)
    else
