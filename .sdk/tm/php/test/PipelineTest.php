...
// CrisisCoreFusion SDK pipeline test
//
...
{
    private static function utility(): CrisisCoreFusionUtility
    {
        return new CrisisCoreFusionUtility();
    }
    ...
    private static function ctx(array $over = []): CrisisCoreFusionContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CrisisCoreFusionContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CrisisCoreFusionError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CrisisCoreFusionMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CrisisCoreFusionMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CrisisCoreFusionMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CrisisCoreFusionSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CrisisCoreFusionMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CrisisCoreFusionResponse([]),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        [, $err] = CrisisCoreFusionMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec([]),
            'response' => null,
            'result' => new CrisisCoreFusionResult([]),
        ]);
        [, $err] = CrisisCoreFusionMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec(['step' => 's']),
            'response' => new CrisisCoreFusionResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        [, $err] = CrisisCoreFusionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec(['step' => 's']),
            'response' => new CrisisCoreFusionResponse(self::resp(200, ['v' => 1])),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        [, $err] = CrisisCoreFusionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec(['step' => 's']),
            'response' => new CrisisCoreFusionResponse(self::resp(200, ['v' => 2])),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CrisisCoreFusionMakeResponse::call($ctx);
    ...
    {
        $preset = new CrisisCoreFusionResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec([]),
            'response' => new CrisisCoreFusionResponse([]),
            'result' => new CrisisCoreFusionResult([]),
        ...
        [$got, $err] = CrisisCoreFusionMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CrisisCoreFusionResult([])]);
        [, $err] = CrisisCoreFusionMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CrisisCoreFusionSpec([]), 'result' => null]);
        [, $err] = CrisisCoreFusionMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CrisisCoreFusionSpec(['step' => 's']),
            'result' => new CrisisCoreFusionResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CrisisCoreFusionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CrisisCoreFusionSpec(['step' => 's']),
            'result' => new CrisisCoreFusionResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CrisisCoreFusionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CrisisCoreFusionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CrisisCoreFusionResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CrisisCoreFusionSpec([]),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CrisisCoreFusionMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CrisisCoreFusionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CrisisCoreFusionError('boom', 'boom');
        $utility->fetcher = function (CrisisCoreFusionContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CrisisCoreFusionSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CrisisCoreFusionMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CrisisCoreFusionContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CrisisCoreFusionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CrisisCoreFusionResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CrisisCoreFusionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CrisisCoreFusionContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CrisisCoreFusionSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CrisisCoreFusionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CrisisCoreFusionResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CrisisCoreFusionSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CrisisCoreFusionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CrisisCoreFusionMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CrisisCoreFusionMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CrisisCoreFusionResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CrisisCoreFusionDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CrisisCoreFusionResult(['ok' => false])]);
        $this->expectException(CrisisCoreFusionError::class);
        CrisisCoreFusionDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CrisisCoreFusionResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CrisisCoreFusionMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CrisisCoreFusionMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CrisisCoreFusionBaseFeature();
        $b = new CrisisCoreFusionBaseFeature();
        CrisisCoreFusionFeatureAdd::call($ctx, $a);
        CrisisCoreFusionFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CrisisCoreFusionBaseFeature
    {
        $f = new CrisisCoreFusionBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CrisisCoreFusionFeatureAdd::call($ctx, self::named_feature('a'));
        CrisisCoreFusionFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CrisisCoreFusionFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CrisisCoreFusionFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CrisisCoreFusionFeatureAdd::call($ctx, $replace);
        ...
        CrisisCoreFusionFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CrisisCoreFusionContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CrisisCoreFusionSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CrisisCoreFusionPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CrisisCoreFusionPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CrisisCoreFusionResponse(['headers' => null]),
            'result' => new CrisisCoreFusionResult([]),
        ]);
        CrisisCoreFusionResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CrisisCoreFusionResultBody::call($ctx);
