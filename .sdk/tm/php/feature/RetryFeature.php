...
// CrisisCoreFusion SDK retry feature
...
class CrisisCoreFusionRetryFeature extends CrisisCoreFusionBaseFeature
{
    ...
    public function init(CrisisCoreFusionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CrisisCoreFusionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CrisisCoreFusionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CrisisCoreFusionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
