...
func TestMateriaDirect(t *testing.T) {
	t.Run("direct-list-materia", func(t *testing.T) {
		setup := materiaDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-materia", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-materia", func(t *testing.T) {
		setup := materiaDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-materia", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type materiaDirectSetupResult struct {
	client *sdk.CrisisCoreFusionSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func materiaDirectSetup(mockres any) *materiaDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CRISISCOREFUSION_TEST_MATERIA_ENTID"]; ok {
		...
		return &materiaDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &materiaDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
