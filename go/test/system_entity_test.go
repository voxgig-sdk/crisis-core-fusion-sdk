...
func TestSystemEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CRISISCOREFUSION_TEST_SYSTEM_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		systemRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.system", setup.data)))
...
func systemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CRISISCOREFUSION_TEST_SYSTEM_ENTID")
