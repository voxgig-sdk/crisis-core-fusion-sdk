...
class MateriaEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CrisisCoreFusionSDK::test(null, null);
        $ent = $testsdk->Materia(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = CrisisCoreFusionSDK::test($seed, null);
        $seen = iterator_to_array($base->Materia(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = CrisisCoreFusionConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = CrisisCoreFusionSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Materia(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = materia_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "materia." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CRISISCOREFUSION_TEST_MATERIA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $materia_ref01_data = null;
        if (count($materia_ref01_data_raw) > 0) {
...
function materia_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/materia/MateriaTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["materia01", "materia02", "materia03"] as $k) {
    ...
    $entid_env_raw = getenv("CRISISCOREFUSION_TEST_MATERIA_ENTID");
    ...
    if ($env["CRISISCOREFUSION_TEST_LIVE"] === "TRUE") {
        ...
        $client = new CrisisCoreFusionSDK(Helpers::to_map($merged_opts));
    }
