...
// CrisisCoreFusion SDK feature test
//
...
class FtCtrl extends CrisisCoreFusionControl
{
...
class FtClient
{
    ...
    private CrisisCoreFusionUtility $utility;

    public function __construct(CrisisCoreFusionUtility $utility, array $options)
    {
    ...
    public function get_utility(): CrisisCoreFusionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CrisisCoreFusionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new CrisisCoreFusionUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (CrisisCoreFusionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CrisisCoreFusionConfig::make_feature($name);
    ...
    private static function buildUrl(CrisisCoreFusionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CrisisCoreFusionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CrisisCoreFusionResponse($fetched);
        }
    ...
    private function fail(CrisisCoreFusionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CrisisCoreFusionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
