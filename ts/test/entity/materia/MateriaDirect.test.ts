...
import { CrisisCoreFusionSDK } from '../../..'
...
describe('MateriaDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CrisisCoreFusionSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-load-materia', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-materia', setup.live)) return
  ...
  test('direct-list-materia', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-materia', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CRISISCOREFUSION_TEST_MATERIA_ENTID']
