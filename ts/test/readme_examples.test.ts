...
//     (\`new CrisisCoreFusionSDK(...)\` / \`CrisisCoreFusionSDK.test(...)\` become
//     \`CrisisCoreFusionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CrisisCoreFusionSDK'
...
const TEST_SEED = {"entity":{"fusion":{"test01":{"id":"test01"}},"materia":{"test01":{"id":"test01"}},"system":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CrisisCoreFusionSDK(...)\` and any
// \`CrisisCoreFusionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CrisisCoreFusionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CrisisCoreFusionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
