...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.CRISISCOREFUSION_TEST_LIVE ||
    'TRUE' === process.env.CRISISCOREFUSION_TEST_OVERRIDE
  ) {
  ...
  m.CRISISCOREFUSION_TEST_EXPLAIN = process.env.CRISISCOREFUSION_TEST_EXPLAIN || m.CRISISCOREFUSION_TEST_EXPLAIN
