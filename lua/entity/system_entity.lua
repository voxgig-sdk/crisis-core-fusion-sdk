...
local SystemEntity = {}
SystemEntity.__index = SystemEntity


function SystemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SystemEntity)
  self._name = "system"
  self._client = client
...
function SystemEntity:get_name()
  return self._name
end


function SystemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SystemEntity.new(self._client, opts)
end


function SystemEntity:data_set(args)
  if args ~= nil then
...
function SystemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SystemEntity:match_set(args)
  if args ~= nil then
...
function SystemEntity:match_get()
...
function SystemEntity:stream(action, args, callopts)
...
---@param reqmatch SystemLoadMatch
---@param ctrl? table
---@return System
---@return string? err
function SystemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SystemEntity:_run_op(ctx, post_done)
  local utility = self._utility
