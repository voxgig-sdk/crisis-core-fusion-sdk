...
local FusionEntity = {}
FusionEntity.__index = FusionEntity


function FusionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FusionEntity)
  self._name = "fusion"
  self._client = client
...
function FusionEntity:get_name()
  return self._name
end


function FusionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FusionEntity.new(self._client, opts)
end


function FusionEntity:data_set(args)
  if args ~= nil then
...
function FusionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FusionEntity:match_set(args)
  if args ~= nil then
...
function FusionEntity:match_get()
...
function FusionEntity:stream(action, args, callopts)
...
function FusionEntity:_run_op(ctx, post_done)
  local utility = self._utility
