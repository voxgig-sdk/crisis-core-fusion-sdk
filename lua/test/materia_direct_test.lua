...
describe("MateriaDirect", function()
  it("should direct-list-materia", function()
    local setup = materia_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-materia", setup.live and "live" or "unit")
  ...
  it("should direct-load-materia", function()
    local setup = materia_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-materia", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function materia_direct_setup(mockres)
