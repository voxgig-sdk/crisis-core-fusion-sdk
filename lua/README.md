...
The Lua SDK for the CrisisCoreFusion API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:Fusion(nil):create({ name = "Example" }, nil)
...
local result, err = client:CrisisCoreFusion(nil):load(
  { id = "test01" }, nil
)
