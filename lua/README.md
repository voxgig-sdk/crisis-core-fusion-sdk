# CrisisCoreFusion Lua SDK



The Lua SDK for the CrisisCoreFusion API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Fusion()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/crisis-core-fusion-sdk/releases)),
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local fusion, err = client:Fusion():load()
    if err then error(err) end
...
```
lua/
├── crisis-core-fusion_sdk.lua    -- Main SDK module
...
The main module (`crisis-core-fusion_sdk`) exports the SDK constructor
