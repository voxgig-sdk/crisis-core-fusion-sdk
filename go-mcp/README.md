...
[MCP](https://modelcontextprotocol.io) server exposing the CrisisCoreFusion SDK as
two agent tools — `crisis-core-fusion_list` and `crisis-core-fusion_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crisis-core-fusion-mcp)
make build

# 2. Provide credentials via the environment
export CRISIS_CORE_FUSION_APIKEY=sk_live_xxx
...
```jsonc
// crisis-core-fusion_list: first page of records
{ "entity": "materia" }
{ "entity": "materia", "query": { } }

// crisis-core-fusion_load: one record by id
{ "entity": "materia", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crisis-core-fusion-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user crisis-core-fusion \
     -- "$PWD"/dist/*/crisis-core-fusion-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `crisis-core-fusion_list` and `crisis-core-fusion_load` tools now appear
   in new sessions. Ask the agent to *"list materia using crisis-core-fusion"*
   and it calls `crisis-core-fusion_list` with `{"entity":"materia"}`.
...
```sh
export CRISIS_CORE_FUSION_APIKEY=sk_live_xxx            # API key
export CRISIS_CORE_FUSION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `crisis-core-fusion_list` tool
...
### Call the `crisis-core-fusion_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `crisis-core-fusion_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `crisis-core-fusion_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CRISIS_CORE_FUSION_APIKEY` | API key sent with every request. |
| `CRISIS_CORE_FUSION_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"crisis-core-fusion_load","arguments":{"entity":"materia","query":{"id":1}}}}'
```
