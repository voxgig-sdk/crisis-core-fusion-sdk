...
class SystemEntity
  ...
  def make
    opts = @_entopts.dup
    SystemEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CrisisCoreFusionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [System, Hash] the current System data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CrisisCoreFusionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of System fields)
  def match_get
  ...
  #
  # @param reqmatch [SystemLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.System.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [System, Hash] the loaded System; raises CrisisCoreFusionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CrisisCoreFusionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
