...
class MateriaEntity
  ...
  def make
    opts = @_entopts.dup
    MateriaEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Materia.load works with no args).
  ...
  #
  # @param reqmatch [MateriaListMatch, Hash, nil] match filter (any subset of
  #   Materia fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
