...
class FusionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = fusion_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "fusion." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CRISISCOREFUSION_TEST_FUSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    fusion_ref01_ent = client.Fusion(nil)
    fusion_ref01_data = Helpers.to_map(Vs.getprop(
      Vs.getpath(setup[:data], "new.fusion"), "fusion_ref01"))

    fusion_ref01_data_result, err = fusion_ref01_ent.create(fusion_ref01_data, nil)
    assert_nil err
    fusion_ref01_data = Helpers.to_map(fusion_ref01_data_result)
    assert !fusion_ref01_data.nil?

  end
end

def fusion_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "fusion", "FusionTestData.json")
  ...
  entid_env_raw = ENV["CRISISCOREFUSION_TEST_FUSION_ENTID"]
