...
class MateriaEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = CrisisCoreFusionSDK.test(seed, nil)
    seen = base.Materia(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Materia(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    materia_ref01_list_result = materia_ref01_ent.list(materia_ref01_match, nil)
    ...
    materia_ref01_data_dt0_loaded = materia_ref01_ent.load(materia_ref01_match_dt0, nil)
