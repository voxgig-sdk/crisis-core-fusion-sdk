...
class TestMateriaEntity:
    ...
    def test_should_stream(self):
        ...
        base = CrisisCoreFusionSDK.test(seed, None)
        seen = list(base.Materia(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CrisisCoreFusionSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Materia(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _materia_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "materia." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CRISISCOREFUSION_TEST_MATERIA_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        materia_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.materia")))
        materia_ref01_data = None
        if len(materia_ref01_data_raw) > 0:
...
def _materia_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/materia/MateriaTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CRISISCOREFUSION_TEST_MATERIA_ENTID"))
