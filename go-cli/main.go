// Command CrisisCoreFusion-cli is a CLI + REPL for the CrisisCoreFusion SDK,
...
const prompt = "crisis-core-fusion"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "fusion materia system"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CRISIS_CORE_FUSION_APIKEY (api key), CRISIS_CORE_FUSION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CRISIS_CORE_FUSION_APIKEY carries the API key and
	// CRISIS_CORE_FUSION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CRISIS_CORE_FUSION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CRISIS_CORE_FUSION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCrisisCoreFusionSDK(opts)
