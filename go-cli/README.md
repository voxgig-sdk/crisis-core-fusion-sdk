...
AQL-driven command-line client **and** interactive REPL for the CrisisCoreFusion
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crisis-core-fusion-cli)
make build

# 2. See usage (words, entities, env vars)
./crisis-core-fusion-cli --help
...
CRISIS_CORE_FUSION_BASE=https://api.example.com ./crisis-core-fusion-cli --help

# 6. No arguments -> interactive REPL
./crisis-core-fusion-cli
crisis-core-fusion> /help
crisis-core-fusion> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crisis-core-fusion-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/crisis-core-fusion-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `crisis-core-fusion>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `fusion`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/crisis-core-fusion-cli`. |
...
(`./crisis-core-fusion-cli <expr>`) and interactively (the REPL), and expressions compose the
